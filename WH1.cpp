...
IMPLEMENT_DYNCREATE(CWH1, CInspWH)

CWH1::CWH1()
{
	// CPI9000Srn
	m_lStnErrorCode		= IDS_INSPWH1_NOT_SELECTED_ERR;
	m_lModSelectMode	= GLASS_MOD;

	// CPRTaskStn
	m_szPRTaskStnName	= GetStation().Trim('_');
	m_lPRStnErrorCode	= IDS_INSPWH1_PR_MOD_NOT_SELECTED;
	...
	// Motor Related
	m_stMotorY.InitMotorInfo("WH1_Y" , &m_bModSelected, "WH1_Y_ENC_PORT_1");
	//m_stMotorX.InitMotorInfo("WH1_X" , &m_bModSelected);
	m_stMotorT.InitMotorInfo("WH1_T", &m_bModSelected);
//#ifdef SRCH_EMBEDDED_SENSOR //20170810
//	m_stMotorT.SetMtrSnrPortName(CString("WH1_T_MTRSNR_PORT"), TRUE, TRUE); //channel has embeded limit snr and indexer inside the encoder plug. An external limit sensor as home sensor
//#endif
	...
	// Si Related
	m_stTLmtSnr		= CSiInfo("INSPWH1TLmtSnr",		"IOMODULE_IN_PORT_3", BIT_0, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stTLmtSnr);
	m_stYLmtSnr			= CSiInfo("INSPWH1YLmtSnr",		"IOMODULE_IN_PORT_3", BIT_1, ACTIVE_HIGH);
	m_mySiList.Add((CSiInfo*)&m_stYLmtSnr);

	m_stGlass1VacSnr		= CSiInfo("INSPWH1G1VacSnr",		"IOMODULE_IN_PORT_3", BIT_5, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stGlass1VacSnr);	
	m_stGlass1AuxVacSnr		= CSiInfo("INSPWH1G1AuxVacSnr",		"IOMODULE_IN_PORT_3", BIT_6, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stGlass1AuxVacSnr);	
	m_stGlass2VacSnr		= CSiInfo("INSPWH1G2VacSnr",		"IOMODULE_IN_PORT_3", BIT_7, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stGlass2VacSnr);
	m_stGlass2AuxVacSnr		= CSiInfo("INSPWH1G2AuxVacSnr",		"IOMODULE_IN_PORT_3", BIT_8, ACTIVE_LOW);
	m_mySiList.Add((CSiInfo*)&m_stGlass2AuxVacSnr);

	// So Related
	m_stGlass1VacSol		= CSoInfo("INSPWH1G1VacSol",		"IOMODULE_OUT_PORT_4", BIT_0, INSPWH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stGlass1VacSol);
	m_stGlass1AuxVacSol		= CSoInfo("INSPWH1G1AuxVacSol",		"IOMODULE_OUT_PORT_4", BIT_1, INSPWH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stGlass1AuxVacSol);
	m_stWeakBlowSol			= CSoInfo("INSPWH1G1WeakBlowSol",		"IOMODULE_OUT_PORT_4", BIT_2, INSPWH_SOL, ACTIVE_HIGH);
	m_mySoList.Add((CSoInfo*)&m_stWeakBlowSol);
	m_stGlass2VacSol		= CSoInfo("INSPWH1G2VacSol",		"IOMODULE_OUT_PORT_4", BIT_3, INSPWH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stGlass2VacSol);
	m_stGlass2AuxVacSol		= CSoInfo("INSPWH1G2AuxVacSol",		"IOMODULE_OUT_PORT_4", BIT_4, INSPWH_SOL, ACTIVE_LOW);
	m_mySoList.Add((CSoInfo*)&m_stGlass2AuxVacSol);
	m_stWeakBlow2Sol		= CSoInfo("INSPWH1G2WeakBlowSol",		"IOMODULE_OUT_PORT_4", BIT_5, INSPWH_SOL, ACTIVE_HIGH);
	m_mySoList.Add((CSoInfo*)&m_stWeakBlow2Sol);

#ifdef MB_FORCE_ADC
	m_stADCPort					= CPortInfo("WH1ADCPort",			"IOMODULE_ADC_PORT_0", BIT_0);
#endif
...
VOID CWH1::RegisterVariables()
{
	try
	{
		...
		// -------------------------------------------------------
		RegVariable(_T("HMI_bWH1YOn"), &m_stMotorY.bHMI_On);
		//RegVariable(_T("HMI_bInspWHZ1On"), &m_stMotorZ[WHZ_1].bHMI_On);
		//RegVariable(_T("HMI_bInspWHZ2On"), &m_stMotorZ[WHZ_2].bHMI_On);
		RegVariable(_T("HMI_bWH1TOn"), &m_stMotorT.bHMI_On);
		RegVariable(_T("HMI_lWH1YEncoder"), &m_stMotorY.lHMI_CurrPosn);
		RegVariable(_T("HMI_lWH1YEncoder2nd"), &m_stMotorY.lHMI_Curr2ndPosn);
		...
		RegVariable(_T("HMI_lWH1TEncoder"), &m_stMotorT.lHMI_CurrPosn);

		RegVariable(_T("HMI_bInspWH1G1VacSnr"), &m_stGlass1VacSnr.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G2VacSnr"), &m_stGlass2VacSnr.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G1AuxVacSnr"), &m_stGlass1AuxVacSnr.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G2AuxVacSnr"), &m_stGlass2AuxVacSnr.bHMI_Status);

		RegVariable(_T("HMI_bInspWH1TLmtSnr"), &m_stTLmtSnr.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1YLmtSnr"), &m_stYLmtSnr.bHMI_Status);
		//RegVariable(_T("HMI_bInspWHZ1LmtSnr"), &m_stZ1LmtSnr.bHMI_Status);
		//RegVariable(_T("HMI_bInspWHZ2LmtSnr"), &m_stZ2LmtSnr.bHMI_Status);


		RegVariable(_T("HMI_bInspWH1G1VacSol"), &m_stGlass1VacSol.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G2VacSol"), &m_stGlass2VacSol.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G1AuxVacSol"), &m_stGlass1AuxVacSol.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1G2AuxVacSol"), &m_stGlass2AuxVacSol.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1WeakBlowSol"), &m_stWeakBlowSol.bHMI_Status);
		RegVariable(_T("HMI_bInspWH1WeakBlow2Sol"), &m_stWeakBlow2Sol.bHMI_Status);
		...
		RegVariable(GetStation() + _T("bG1AlignPtSet"),	&m_stInspWH[GLASS1].bAlignPtSet);
		RegVariable(GetStation() + _T("bG1Valid"),	&m_stInspWH[GLASS1].bValid);
		RegVariable(GetStation() + _T("bG2AlignPtSet"),	&m_stInspWH[GLASS2].bAlignPtSet);
		RegVariable(GetStation() + _T("bG2Valid"),	&m_stInspWH[GLASS2].bValid);
...
#ifdef MB_FORCE_ADC
		RegVariable(_T("HMI_lWH1ADCForce"), &m_lForceADCValue);

		//RegVariable(_T("HMI_dMB1ADCForceWeight"), &m_dForceBondWeight);

		//RegVariable(_T("HMI_dMB1ADCForceSnrFactor"), &m_dForceSensorFactor);
